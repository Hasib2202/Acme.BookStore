...
export class AuthorComponent implements OnInit {
  ...
  ngOnInit() {
    const authorStreamCreator = (query: any) => this.authorService.getList(query);
  ...
  buildForm() {
    this.form = this.fb.group({
      ...
      email: [this.selectedAuthor.email || '', Validators.required],
      bio: [this.selectedAuthor.bio || ''],
      dateOfBirth: [
        this.selectedAuthor.dateOfBirth
          ? this.parseDate(this.selectedAuthor.dateOfBirth)
          : null,
      ],
    });
  }
  ...
  save() {
    ...
    if (this.selectedAuthor.id) {
      request = this.authorService.update(this.selectedAuthor.id, requestData);
    }
